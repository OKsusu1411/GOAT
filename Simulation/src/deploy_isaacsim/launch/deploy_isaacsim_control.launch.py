# deploy_isaacsim_control.launch.py
#
# Isaac Sim control bringup: topic_converter_node + sim_controller_node.
#
# Prerequisite: Isaac Sim must already be publishing /sim_joint_states, /sim_imu, /sim_odom.
#
# This workspace is an OVERLAY on the Realworld workspace, which owns the shared
# control core (goat_control.utils.controller), the YAML config, and the policy
# checkpoints. Source it first:
#   cd Realworld  && colcon build && source install/setup.bash
#   cd Simulation && colcon build && source install/setup.bash
#   ros2 launch deploy_isaacsim deploy_isaacsim_control.launch.py
#
# Visualization (robot_state_publisher + rviz) is intentionally NOT included here.
# Launch it separately when needed:
#   ros2 launch goat_description display.launch.py use_sim_time:=true use_gui:=false

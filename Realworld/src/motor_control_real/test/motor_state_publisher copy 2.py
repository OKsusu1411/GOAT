...
class MotorStatePublisher(Node):
    ...
    def _txrx(self, node_id: int, cmd: int, payload7: bytes=b'\x00'*7, timeout=0.05):
        ...
        - 같은 TX ID라 하더라도 '내용이 다른' 프레임은 유효 응답으로 인정
        - (필요시 RX ID도 허용하려면 주석 부분을 활성화)
        """
        ...
        while time.time() < t_end:
            ...
            # # 1) 정상 프로토콜 (0x180+ID) 응답 허용하려면 사용
            # if m.arbitration_id == rx_id:
            #     return m

            # 2) 같은 ID(0x140+ID)로 응답하는 경우 허용 (TX-echo 구분)
            if m.arbitration_id == tx_id:
                # loopback 에코는 보낸 프레임과 동일 → 무시
                if m.data == data:
                    continue
                # 내용이 다르면 실제 응답으로 간주
                return m
    ...
    def poll_single_or_multi_turn(self, motor_idx: int):
        ...
        if motor_idx in self.single_turn_indices:
            ...
            if not rep:
                return
            d = rep.data
            # 0x94: DATA[4..7] = uint32 LE (0.01°/LSB)
            ...
            s64  = int.from_bytes(raw7 + sign, byteorder='little', signed=True)

...
SCALE_A_PER_LSB = {
    "MF": 16.5 / 2048.0,  # MF 시리즈: 약 0.00806 A/LSB (±16.5A 범위)
    "MG": 33.0 / 2048.0,   # MG 시리즈: 약 0.01611 A/LSB (±33.0A 범위)
}

class MotorTorqueController(Node):
    def __init__(self):
        ...
        self.series = self.declare_parameter('series', 'MF').value
        ...
        # 모터 시리즈에 따른 전류 스케일 설정
        if self.series in SCALE_A_PER_LSB:
            self.scale = SCALE_A_PER_LSB[self.series]
        else:
            self.get_logger().error(f"Unknown series '{self.series}'. Must be 'MF' or 'MG'. Defaulting to 'MF'.")
            self.scale = SCALE_A_PER_LSB['MF']
    ...
    def _send_command_expect(self, node_id: int, cmd_byte: int, payload7: bytes = b"\x00" * 7):
        ...
        while time.time() < end_time:
            ...
            if rx_msg is None:
                continue
            if rx_msg.arbitration_id == rx_id and len(rx_msg.data) == 8 and rx_msg.data[0] == cmd_byte:
                return rx_msg
        return None

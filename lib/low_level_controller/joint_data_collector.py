...
class PD_Controller():
    ...
    def __init__(self, kp, kd, num_envs: int, num_dof: int, device: str, dt: float):
        ...
        coulomb_coeffs = torch.tensor([HIP_COL_FRI, HIP_COL_FRI, KNEE_COL_FRI], device=device).unsqueeze(0)
        viscous_coeffs = torch.tensor([HIP_VIS_FRI, HIP_COL_FRI, KNEE_VIS_FRI], device=device).unsqueeze(0)
    ...
    def compute_torque(
        ...
        torque = torch.round(torque*100)/100
...
#    DEFAULT_KD_LIST         = [0.010, 0.010, 0.010,  0.010,  0.0001,  0.0001, 0.026, 0.01183]
    ...
    while t <= sim_len:
        ...
        j_target = reference_angle[env_idx].cpu().numpy() # Target Position
        ...
        for i in range(8):
            ...
            data_row[f"target_value_{i}"] = j_target[i]
